...
class EventsITest extends AbstractITestBase {
    ...
    void marketingScenarioRealTime() {
        ...
        for ( int i=0; i < 20; ++i ) {
            Thread.sleep(500);
            resp = alerts.get(path: "", query: [startTime:start,triggerIds:"marketing-scenario"] )
            if ( resp.status == 200 && resp.data != null && resp.data.size == 4) {
                break;
            }
            assertEquals(200, resp.status)
        }
    ...
    void fraudScenarioDslRealTime() {
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e1, e6, e11))
        assertEquals(200, resp.status)
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e2, e12))
        assertEquals(200, resp.status)
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e3, e13))
        assertEquals(200, resp.status)
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e4, e14))
        assertEquals(200, resp.status)
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e5, e15))
        assertEquals(200, resp.status)
        ...
        for ( int i=0; i < 20; ++i ) {
            Thread.sleep(500);
            resp = alerts.get(path: "", query: [startTime:start,triggerIds:"fraud-scenario"] )
    ...
    void customerRetentionScenarioDslRealTime() {
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e1, e11))
        assertEquals(200, resp.status)
        ...
        Event e14 = new Event(TEST_TENANT, uuid(), now(), DATA_ID, "Stock Check", "Out of Stock");
        ...
        alerter.post(path: "events/data", body: Arrays.asList(e6, e16))
        assertEquals(200, resp.status)

        // The alert processing happens async, so give it a little time before failing...
        for ( int i=0; i < 20; ++i ) {
            Thread.sleep(500);
            resp = alerts.get(path: "", query: [startTime:start,triggerIds:"customer-retention-scenario"] )
